  ...
  if (card.tcgplayer !== undefined) {
    return (
      <div className="row align-items-start">
        ...
        {card.tcgplayer && Object.keys(card.tcgplayer.prices)
          .map((priceType: string, index: number) => (
            <div key={index} className="d-flex justify-content-between">
              ...
              <span className="small text-muted">
                {card.tcgplayer!.prices[priceType as keyof typeof card.tcgplayer.prices]?.market ? `$${card.tcgplayer!.prices[priceType as keyof typeof card.tcgplayer.prices]?.market}` : 'N/A'}
              </span>
            </div>
          ))}
      </div>
  ...
  if (card.cardmarket != undefined) {
    return (
      <div className="row align-items-start">
        ...
        {card.cardmarket && Object.keys(card.cardmarket.prices)
          .filter(priceType => card.cardmarket!.prices[priceType as keyof typeof card.cardmarket.prices] !== 0)
          ...
          .map((priceType, index) => (
            <div key={index} className="d-flex justify-content-between">
              <div className="small text-primary-emphasis">
                {card.set.name}
                <span
                  className="text-muted"> – {priceType.charAt(0).toUpperCase() + priceType.slice(1)}</span>
              </div>
              <span className="small text-muted">
                {card.cardmarket!.prices[priceType as keyof typeof card.cardmarket.prices] ? `€${card.cardmarket!.prices[priceType as keyof typeof card.cardmarket.prices]}` : 'N/A'}
              </span>
            </div>
          ))}
      </div>
...
const CardInfo: React.FC = () => {
  ...
  useEffect(() => {
    if (cardId) {
      fetch(`${config.host}/api/v1/get/${cardId}`)
                    ...
                    <div key={index} className="py-1">
                      <div className={"row"}>
                        <div className="col">
                          ...
                          <h6
                            className="card-title d-inline-block me-2">{ability.name}</h6>
                        </div>
                      </div>
                      <div className={"row"}>
                  ...
                  card.attacks?.map((attack, index) => (
                    <div key={index} className="py-1">
                      <div className={"row"}>
                        <div className="col">
                          <h6
                            className="card-title d-inline-block me-2">{attack.name} {attack.damage ? `[${attack.damage}]` : ""}</h6>
